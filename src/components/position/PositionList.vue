...
const currentRecord = ref<PositionRecord>({})
...
const handleDelete = async (record: any) => {
  try {
    const { data } = await request.delete('/api/position/delete', {
      params: { id: record.id }
    })
    if (data.code === 200) {
...
// 编辑相关的数据和方法
const editableData: UnwrapRef<Record<string, PositionRecord>> = reactive({})

const edit = (record: PositionRecord) => {
  editableData[record.id as number] = cloneDeep(record)
}

const dongjie = async (record: PositionRecord) => {
  const {data} = await request.put('/api/position/dongjie', record)
  if (data.success) {
...
const save = async (record: PositionRecord) => {
  try {
    const row = editableData[record.id as number]
    const { data } = await request.put('/api/position/edit', row)
    if (data.code === 200) {
      message.success('编辑成功')
  ...
  <div class="table-page">
    ...
    >
      <template #bodyCell="{ column, text, record }">
        <template v-if="column.dataIndex === 'positionName' || column.dataIndex === 'address'">
